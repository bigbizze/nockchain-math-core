...
use crate::math_error::{MathError, MathResult};
...
#[inline(always)]
pub fn f6_div(f1: &F6lt, f2: &F6lt) -> MathResult<F6lt> {
...
#[inline(always)]
pub fn f6_inv(f: &F6lt) -> MathResult<F6lt> {
    if f == &F6_ZERO {
...
#[inline(always)]
pub fn ch_double_unsafe(x: &F6lt, y: &F6lt) -> MathResult<CheetahPoint> {
    let slope = f6_div(
...
#[inline(always)]
pub fn ch_double(p: CheetahPoint) -> MathResult<CheetahPoint> {
    if p.inf {
        return Ok(A_ID);
    }
    if p.y == F6_ZERO {
...
#[inline(always)]
pub fn ch_add_unsafe(p: CheetahPoint, q: CheetahPoint) -> MathResult<CheetahPoint> {
...
#[inline(always)]
pub fn ch_add(p: &CheetahPoint, q: &CheetahPoint) -> MathResult<CheetahPoint> {
    if p.inf {
        return Ok(*q);
    }
    if q.inf {
...
#[inline(always)]
pub fn ch_scal(n: u64, p: &CheetahPoint) -> MathResult<CheetahPoint> {
    let mut n = n;
    let mut p_copy = *p;
    let mut acc = A_ID;
    while n > 0 {
...
#[inline(always)]
pub fn ch_scal_big(n: &UBig, p: &CheetahPoint) -> MathResult<CheetahPoint> {
...
mod test {
    use super::*;
    use crate::belt::Belt;
    use crate::math_error::MathResult;
